...
export async function PersonnelCenterPage({
  ...
  const firstAttentionMember = data.membersNeedingAttention[0] ?? null;
  const firstPersonnelAction = data.actions[0] ?? null;
  const firstTransfer = data.pendingTransfers[0] ?? null;
  const firstLoaReturn = data.loaReturns[0] ?? null;
  const nextAction: PersonnelNextAction | null = firstPersonnelAction
    ? {
        href: "/personnel#personnel-actions",
        label: "Open action queue",
        reason: firstPersonnelAction.summary,
        responsible: "S1",
        tone: getPriorityTone(firstPersonnelAction.priority),
      }
    : firstTransfer
      ? {
          href: "/personnel#transfers-and-loa",
          label: "Review transfer",
          reason: `${firstTransfer.memberDisplayName} requested transfer to ${firstTransfer.requestedUnitShortName}.`,
      ...
      : firstLoaReturn
        ? {
            href: "/personnel#transfers-and-loa",
            label: "Review LOA",
            reason: `${firstLoaReturn.memberDisplayName} has an LOA return due ${formatDate(firstLoaReturn.expectedReturnAt)}.`,
            responsible: "Unit Leadership / S1",
            tone: "warning",
          }
        : firstAttentionMember
          ? {
              href: `/personnel/members/${firstAttentionMember.id}`,
              label: "Open member",
              reason: `${firstAttentionMember.displayName} has ${formatCountLabel(firstAttentionMember.reasons.length, "readiness issue")}.`,
  ...
  return (
    <div className="space-y-6">
      ...
      <PersonnelNextActionCard action={nextAction} title="Personnel center next action" />
      <PersonnelHandoffRail
        currentStep={`${data.metrics.pendingPersonnelActions} open action(s)`}
        handoffs={[
          {
            description: "Members submit LOA, transfer, and profile correction requests without seeing restricted staff notes.",
            from: "Member",
            status: "info",
            to: "Unit Leadership",
          },
          {
            description: "Unit leadership confirms operational impact, vacancies, readiness gaps, and attendance concerns.",
            from: "Unit Leadership",
            status: "warning",
            to: "S1",
          },
          {
            description: "S1 applies status, assignment, and profile changes through service-backed roster history.",
            from: "S1",
            status: "success",
            to: "Member Record",
          },
          {
            description: "Training staff resolves qualification and signoff gaps that affect member and unit readiness.",
            from: "Training",
            status: "info",
            to: "Readiness",
          },
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
        <Card className="border-border/80 bg-card/88">
          ...
          <CardContent>
            {data.membersNeedingAttention.length === 0 ? (
              ...
              <CompactList
                items={data.membersNeedingAttention.map((member) => ({
                  actionHref: `/personnel/members/${member.id}`,
                  actionLabel: "Open",
                  meta: `${member.unitShortName ?? "Unassigned"} / ${formatCountLabel(member.reasons.length, "issue")}`,
                  statusLabel: member.unitShortName ?? "No unit",
                  statusTone: member.unitShortName ? "info" : "warning",
                  subtitle: member.reasons.join(" / "),
        ...
        <CollapsibleSection
          badgeLabel={`${data.readinessRules.length} finding(s)`}
          badgeTone={data.readinessRules.length > 0 ? "warning" : "success"}
          description="Explainable member and unit readiness rule results. Scores stay secondary to the reasons."
          title="Rule engine findings"
        >
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <Card className="border-border/80 bg-card/88" id="personnel-actions">
          <CardHeader>
        ...
        <Card className="border-border/80 bg-card/88" id="transfers-and-loa">
          <CardHeader>
