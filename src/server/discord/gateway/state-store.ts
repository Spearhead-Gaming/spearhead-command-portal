...
export async function recordGatewayEvent(input: {
  ...
  const data = {
    ...
    metadata: toJson({
      ...
      discordResourceId: input.event.discordResourceId,
      discordUserId: input.event.discordUserId,
      eventId: input.event.eventId,
      handlerVersion: input.event.handlerVersion,
      safeMetadata: input.event.safeMetadata,
      shardId: input.event.shardId,
