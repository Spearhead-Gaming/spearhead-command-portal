...
Permissions are system-defined and should be seeded by the application.

Roles are admin-created, but starter role presets should be seeded for convenience.

Application logic must never depend on role names. It must check permission keys.
